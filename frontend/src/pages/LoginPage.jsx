...
const LoginPage = () => {
  ...
  const handleAdminLogin = () => {
    setValue('email', 'admin@kongu.edu');
  ...
  const onSubmit = async (data) => {
    setIsLoading(true);
    try {
      // Prevent admin login in student portal
      if (data.email === 'admin@kongu.edu') {
        setError('root', {
          type: 'manual',
          message: 'Admin account cannot login here. Please use Admin Login page.'
        });
        setIsLoading(false);
        return;
      }

      // Validate @kongu.edu domain for students
      if (!data.email.endsWith('@kongu.edu')) {
        setError('root', {
          type: 'manual',
          message: 'Only @kongu.edu email addresses are allowed for student login.'
        });
        setIsLoading(false);
        return;
      }
      ...
      if (result.success) {
        ...
        if (result.user.role === 'admin') {
          // Double check - prevent admin access
          setError('root', {
            type: 'manual',
            message: 'Admin account detected. Please use Admin Login page.'
          });
          setIsLoading(false);
          return;
        } else {
            ...
            {!isAuthenticated && (
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Click to auto-fill admin login details (will be blocked for security)
                </p>
              </div>
            )}
