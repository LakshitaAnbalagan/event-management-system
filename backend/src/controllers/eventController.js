    ...
    const eventsWithStats = await Promise.all(
      events.map(async (event) => {
        ...
        return {
          ...
          registrationCount,
          userRegistered,
          isRegistrationOpen: new Date() <= (event.registrationDeadline || event.startDate) && event.isActive,
...
const getEventById = async (req, res) => {
  try {
    ...
    if (req.user) {
      userRegistration = registrations.find(reg => reg.user._id.toString() === req.user._id.toString());
    }
    ...
    const eventWithStats = {
      ...
      isRegistrationOpen: new Date() <= (event.registrationDeadline || event.startDate) && event.isActive,
      isFull: event.maxParticipants ? registrations.length >= event.maxParticipants : false,
